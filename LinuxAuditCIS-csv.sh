...
echo "1.1 Configuracao do Sistema de Arquivos"
echo "1.1.1 Desativar sistemas de arquivos nao utilizados"
...
##################################################################################
CONTROL="1.1.13 Certifique-se de que a opcao nodev esteja configurada na particao home / home"
mount | grep /home
if [ "$?" == "0" ]; then
  echo "$CONTROL,pass">> $LOG
  else
...
##################################################################################
CONTROL="1.1.14 Certifique-se de que a opcao nodev esteja definida na particao / dev / shm"
mount | grep /home
if [ "$?" == "0" ]; then
  echo "$CONTROL,pass">> $LOG
  else
